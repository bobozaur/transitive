...
pub fn from_process_attr(name: &Ident, attr: Attribute) -> Option<SynResult<TokenStream>> {
...
fn process_transitive_attr(name: &Ident, arg_list: ArgList) -> SynResult<TokenStream> {
    ...
    stmts.extend(quote! {let interm = #first::into(val);});
    stmts.extend(quote! {let interm = #last::into(interm);});
...
fn process_transitive_all_attr(name: &Ident, arg_list: ArgList) -> SynResult<TokenStream> {
    ...
    impls.extend(create_from_impl(name, &last, &first));
